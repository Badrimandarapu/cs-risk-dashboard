...
const ACCOUNTS = [
  ...
  { id: 6, name: 'Retail Dynamics Inc', industry: 'Retail', tier: 'Mid-Market', health: 51, users: 340, mrr: 15000, status: 'warning' },
]
    ...
    <div style={{ minHeight: '100vh', backgroundColor: '#0f172a', color: '#fff' }}>
      ...
      <div style={{ maxWidth: '1280px', margin: '0 auto', padding: '2rem', display: 'grid', gap: '2rem' }}>
        ...
        <div style={{ backgroundColor: '#1e293b', border: '1px solid #334155', borderRadius: '8px', overflow: 'hidden' }}>
          <div style={{ padding: '1.5rem', borderBottom: '1px solid #334155', backgroundColor: '#0f172a', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <h3 style={{ fontSize: '1.125rem', fontWeight: '600' }}>Accounts</h3>
              <p style={{ fontSize: '0.875rem', color: '#94a3b8', marginTop: '0.25rem' }}>{ACCOUNTS.length} total accounts</p>
            </div>
            <div style={{ display: 'flex', gap: '0.5rem' }}>
              <button style={{ padding: '0.5rem', backgroundColor: 'transparent', border: 'none', cursor: 'pointer', color: '#94a3b8' }}>🔍</button>
              <button style={{ padding: '0.5rem', backgroundColor: 'transparent', border: 'none', cursor: 'pointer', color: '#94a3b8' }}>⚙️</button>
            </div>
          </div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {ACCOUNTS.map((account) => (
                  <tr key={account.id} style={{ borderBottom: '1px solid #334155', cursor: 'pointer' }} onMouseEnter={(e) => e.currentTarget.style.backgroundColor = 'rgba(51, 65, 85, 0.3)'} onMouseLeave={(e) => e.currentTarget.style.backgroundColor = 'transparent'}>
                    <td style={{ padding: '1rem', fontSize: '0.875rem', fontWeight: '500' }}>
                      <div>{account.name}</div>
                      <div style={{ fontSize: '0.75rem', color: '#64748b' }}>{account.industry}</div>
                    </td>
