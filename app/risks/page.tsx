...
const RISKS = [
  ...
  { id: 4, account: 'Retail Dynamics', type: 'Renewal Risk', severity: 'high', probability: 65, description: 'Renewal in 45 days, health declining, unlikely to renew' },
]
        ...
        <div style={{ display: 'grid', gap: '1rem' }}>
          {RISKS.map((risk) => (
            <div key={risk.id} style={{
              ...
              borderRadius: '8px',
              padding: '1.5rem',
              borderLeft: `4px solid ${risk.severity === 'critical' ? '#ef4444' : risk.severity === 'high' ? '#f59e0b' : '#3b82f6'}`
            }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start' }}>
                <div>
                  <h3 style={{ fontWeight: '600', fontSize: '1rem' }}>{risk.type}</h3>
                  <p style={{ color: '#94a3b8', fontSize: '0.875rem', marginTop: '0.5rem' }}>{risk.description}</p>
                  <p style={{ color: '#64748b', fontSize: '0.75rem', marginTop: '0.5rem' }}>Account: {risk.account}</p>
                </div>
                <div style={{ textAlign: 'right' }}>
                  <p style={{ fontSize: '1.875rem', fontWeight: 'bold', color: risk.severity === 'critical' ? '#ef4444' : risk.severity === 'high' ? '#f59e0b' : '#3b82f6' }}>
                    {risk.probability}%
                  </p>
                  <span style={{
                    ...
                    fontSize: '0.75rem',
                    fontWeight: '600',
                    backgroundColor: risk.severity === 'critical' ? '#7f1d1d' : risk.severity === 'high' ? '#78350f' : '#0c4a6e',
                    color: risk.severity === 'critical' ? '#fca5a5' : risk.severity === 'high' ? '#fcd34d' : '#7dd3fc'
                  }}>
                    {risk.severity}
                  </span>
                </div>
