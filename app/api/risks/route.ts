...
export async function GET() {
  try {
    ...
    const risks = (signals as any[]).map((signal: any) => {
      const severityMap = { CRITICAL: 'critical', HIGH: 'high', MEDIUM: 'medium', LOW: 'low' }
      const probabilityMap = { CRITICAL: 95, HIGH: 75, MEDIUM: 50, LOW: 25 }
      ...
      return {
        ...
        probability: Math.round(probabilityMap[signal.severity] * signal.confidence),
