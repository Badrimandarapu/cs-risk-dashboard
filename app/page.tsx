...
import { mockAccounts, mockInsights } from '@/lib/mockData'
...
export default function Home() {
  const healthyCount = mockAccounts.filter((a) => a.riskLevel === 'green').length
  const atRiskCount = mockAccounts.filter((a) => a.riskLevel === 'red').length
  const avgScore = Math.round(mockAccounts.reduce((sum, a) => sum + a.healthScore, 0) / mockAccounts.length)
  ...
  return (
    <div style={{ padding: '32px' }}>
      ...
      <div style={{ marginBottom: '32px' }}>
        ...
        <p style={{ fontSize: '16px', color: '#6b7280' }}>Real-time risk intelligence powered by AI</p>
      </div>
      ...
      >
        {[
          { label: 'Total Accounts', value: mockAccounts.length, color: '#3b82f6' },
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ borderBottom: '2px solid #e5e7eb', backgroundColor: '#f9fafb' }}>
              ...
              <th style={{ padding: '12px', textAlign: 'left', fontWeight: 'bold', color: '#111827' }}>Action</th>
            </tr>
          </thead>
          <tbody>
            {mockAccounts.map((account) => {
              ...
              return (
                <tr key={account.id} style={{ borderBottom: '1px solid #e5e7eb' }}>
                  ...
                  <td style={{ padding: '12px' }}>
                    <button style={{ color: '#2563eb', fontWeight: '600', cursor: 'pointer', background: 'none', border: 'none' }}>
                      View Details →
                    </button>
                  </td>
                </tr>
